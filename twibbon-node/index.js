...
app.get("/", (req, res) => {
  res.send({ message: "Hello Twibbons" });
});
...
app.get("/db-test", async (req, res) => {
  ...
  try {
    ...
    console.log("ERROR TES DB: ", error);
  } finally {
    if (db) db.release();
  }
});
...
app.get("/api/twibbons", (req, res) => {
  db.query(
    "SELECT * FROM twibbons ORDER BY created_at DESC",
    (err, results) => {
...
app.post("/api/twibbons", upload.single("file"), (req, res) => {
  ...
  db.query(
    "INSERT INTO twibbons (name, description, filename, url) VALUES (?, ?, ?, ?)",
    [name, description, filename, url],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
      db.query(
        "SELECT * FROM twibbons WHERE id = ?",
        [result.insertId],
        (err2, rows) => {
...
app.put("/api/twibbons/:id", (req, res) => {
  ...
  const { name, description } = req.body;
  db.query(
    "UPDATE twibbons SET name = ?, description = ? WHERE id = ?",
    [name, description, id],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
...
app.post("/api/auth/login", (req, res) => {
  ...
  db.query(
    "SELECT * FROM sysuser WHERE username = ? AND is_active = 1",
    [username],
    async (err, rows) => {
      if (err) return res.status(500).json({ error: err.message });
      if (!rows.length)
...
app.post("/api/auth/change-credentials", async (req, res) => {
  ...
  db.query(
    ...
    async (err, rows) => {
      ...
      db.query(
        "UPDATE sysuser SET username = ?, password = ?, updated_at = NOW() WHERE id = ?",
...
app.post("/api/feedback", (req, res) => {
  ...
  db.query(
    "INSERT INTO feedback (message, name, created_at) VALUES (?, ?, NOW())",
    [message, sender],
    (err, result) => {
      if (err) return res.status(500).json({ error: err.message });
      db.query(
